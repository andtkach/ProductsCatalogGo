...
func (s *Store) GetUserByEmail(email string) (*types.User, error) {
	rows, err := s.db.Query("SELECT id, firstName, lastName, email, password, createdAt, active, role FROM users WHERE email = ?", email)
	if err != nil {
		return nil, err
	}
...
func (s *Store) GetUserById(id int) (*types.User, error) {
	rows, err := s.db.Query("SELECT id, firstName, lastName, email, password, createdAt, active, role FROM users WHERE id = ?", id)
	if err != nil {
		return nil, err
	}
